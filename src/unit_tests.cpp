...
int main() {
    ...
    lpn_block03.AddConnectingBlock(name07, -1);
    lpn_block03.AddConnectingWire(name08);
    lpn_block07.AddConnectingWire(name08);
    
    if (lpn_block03.GetConnectingBlockList().size() != connecting_block_list03.size() + 1) {
        std::cout << "Error. lpn_block03.connecting_block_list.size, " << lpn_block03.GetConnectingBlockList().size() << ", does not match connecting_block_list03.size + 1 = " << connecting_block_list03.size() + 1 << ".\n";
    } else {
        if (lpn_block03.GetConnectingBlockList()[connecting_block_list03.size()].compare(name07) != 0) {
            std::cout << "Error. lpn_block03.connecting_block_list[connecting_block_list03.size()], " << lpn_block03.GetConnectingBlockList()[connecting_block_list03.size()] << ", does not match name07, " << name07 << ".\n";
        }
    }
    
    if (lpn_block03.GetFlowDirections().size() != flow_directions03.size() + 1) {
        std::cout << "Error. lpn_block03.flow_directions.size, " << lpn_block03.GetFlowDirections().size() << ", does not match flow_directions03.size + 1 = " << flow_directions03.size() + 1 << ".\n";
    } else {
        if (lpn_block03.GetFlowDirections()[flow_directions03.size()] != -1) {
            std::cout << "Error. lpn_block03.flow_directions[flow_directions03.size()], " << lpn_block03.GetFlowDirections()[flow_directions03.size()] << ", does not equate to 1.\n";
        }
    }
    ...
    // last here - use assert instead of print statements -- works only in debug mode
    // - or throw exceptions
}